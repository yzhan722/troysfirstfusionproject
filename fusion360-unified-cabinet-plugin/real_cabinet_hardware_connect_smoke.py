#!/usr/bin/env python3
"""Real-cabinet hardware Connect smoke â€?Overhead screw + tongue via generic routes.

Install: python scripts/manage_fusion_smokes.py install --batch realhw
"""
...
def run(_context):
    ...
    try:
        ...
        for name in (
            ...
            "overhead_declared_relationships",
            "panel_metadata_writeback",
            "modules.hardware.controller",
            "modules.relationships.controller",
            "modules.overhead.controller",
            "real_cabinet_hardware_fusion_smoke_runner",
        ):
            try:
        ...
        overhead_module = sys.modules["modules.overhead.controller"]
        real_runner = sys.modules["real_cabinet_hardware_fusion_smoke_runner"]
        ...
        oh_ctrl = overhead_module.OverheadController(plugin_dir, fusion)
        result = real_runner.run_real_cabinet_hardware_fusion_smoke(
            plugin_dir, fusion, rel_ctrl, hw_ctrl, oh_ctrl, write_json=True,
        )
        if ui:
            ui.messageBox(format_summary(result))
    except Exception:
        if ui:
            ui.messageBox("Real-cabinet hardware smoke FAILED:\n{}".format(traceback.format_exc()))
        raise

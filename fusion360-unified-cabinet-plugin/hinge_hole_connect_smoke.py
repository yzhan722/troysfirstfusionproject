#!/usr/bin/env python3
"""Hinge hole Connect smoke â€?host cup cut + metadata writeback.

Install: python scripts/manage_fusion_smokes.py install --batch hinge
"""
...
def run(_context):
    ...
    try:
        ...
        for name in (
            ...
            "hinge_hole_fusion_smoke_runner",
        ):
            try:
        ...
        hinge_runner = sys.modules["hinge_hole_fusion_smoke_runner"]
        from connect_smoke_runner import format_summary

        run_hinge_hole_fusion_smoke = hinge_runner.run_hinge_hole_fusion_smoke
        ...
        if not hasattr(hw_ctrl, "create_hinge_holes_from_relationship"):
            raise RuntimeError(
                "HardwareController missing hinge methods after reload; "
                "stop/start CabinetNC add-in, then re-run this smoke."
            )
        result = run_hinge_hole_fusion_smoke(
        ...
        if ui:
            ui.messageBox("Hinge hole smoke FAILED:\n{}".format(traceback.format_exc()))
        raise

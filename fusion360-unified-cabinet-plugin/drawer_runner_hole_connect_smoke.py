#!/usr/bin/env python3
"""Drawer runner hole Connect smoke â€?host mount holes + metadata writeback.

Install: python scripts/manage_fusion_smokes.py install --batch runner
"""
...
def run(_context):
    ...
    try:
        ...
        for name in (
            ...
            "drawer_runner_hole_fusion_smoke_runner",
        ):
            try:
        ...
        runner = sys.modules["drawer_runner_hole_fusion_smoke_runner"]
        from connect_smoke_runner import format_summary

        run_drawer_runner_hole_fusion_smoke = runner.run_drawer_runner_hole_fusion_smoke
        ...
        if not hasattr(hw_ctrl, "create_drawer_runner_holes_from_relationship"):
            raise RuntimeError(
                "HardwareController missing drawer runner methods after reload; "
                "stop/start CabinetNC add-in, then re-run this smoke."
            )
        result = run_drawer_runner_hole_fusion_smoke(
        ...
        if ui:
            ui.messageBox("Drawer runner hole smoke FAILED:\n{}".format(traceback.format_exc()))
        raise

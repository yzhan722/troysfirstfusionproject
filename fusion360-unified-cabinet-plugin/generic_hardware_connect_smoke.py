...
def run(_context):
    ...
    try:
        ...
        for name in (
            ...
            "generic_hardware_fusion_smoke_runner",
        ):
            try:
        ...
        generic_runner = sys.modules["generic_hardware_fusion_smoke_runner"]
        from connect_smoke_runner import format_summary

        run_generic_hardware_fusion_smoke = generic_runner.run_generic_hardware_fusion_smoke
        ...
        if not hasattr(hw_ctrl, "create_hardware_from_relationship"):
            raise RuntimeError(
                "HardwareController missing create_hardware_from_relationship; "
                "stop/start CabinetNC add-in, then re-run this smoke."
            )
        result = run_generic_hardware_fusion_smoke(
        ...
        if ui:
            ui.messageBox("Generic hardware smoke FAILED:\n{}".format(traceback.format_exc()))
        raise

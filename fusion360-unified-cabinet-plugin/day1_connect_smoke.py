#!/usr/bin/env python3
"""Day 1 Connect smoke â€?optional standalone Fusion Script entry point.

Run from Fusion: Scripts and Add-Ins â†?Scripts â†?run this file.
...
def run(_context):
    ...
    try:
        ...
        import adapter
        from day1_connect_smoke_runner import format_summary, run_day1_connect_smoke
        ...
        result = run_day1_connect_smoke(plugin_dir, fusion, rel_ctrl, hw_ctrl, write_json=True)
        if ui:
            ui.messageBox(format_summary(result))
    except Exception:
        if ui:
            ui.messageBox("Day 1 Connect smoke FAILED:\n{}".format(traceback.format_exc()))
        raise
